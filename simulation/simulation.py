...
class Simulation:
    ...
    def __init__(self, config_path: str = "config.yaml", output_dir: Optional[str] = None):
        ...
        self.world_description = sim_config.get('world_description', '')  # Optional override
        ...
        # Validate each place configuration. A place must specify size as either:
        #   (a) `half_size` (square, used for both X/Y), or
        #   (b) both `half_size_x` and `half_size_y` (rectangle).
        required_fields = ['name', 'type', 'center_x', 'center_y', 'capacity']
        for i, place in enumerate(self.places):
            ...
            has_legacy = 'half_size' in place
            has_xy = 'half_size_x' in place and 'half_size_y' in place
            if not has_legacy and not has_xy:
                raise ValueError(
                    f"Place at index {i} must specify either 'half_size' "
                    f"OR both 'half_size_x' and 'half_size_y'."
                )
        ...
        for i, fc in enumerate(fires_config):
            ...
            logger.info(
                f"Hazard '{config_entry['name']}' configured: step={fc['start_step']}, "
                f"intensity={fc['intensity']}, radius={fc['radius']}, position={pos_info}"
            )
        self.fire_states: List[Dict] = []  # Active fires

        # Announcements (PA system messages broadcast at specific steps)
        announcements_config = self.config.get('announcements', [])
        self.announcements: List[Dict] = []
        for ac in announcements_config:
            self.announcements.append({
                'step': ac['step'],
                'message': ac['message'],
            })
        self.announcements.sort(key=lambda a: a['step'])
        self.current_announcement: Optional[str] = None
        self.last_announcement_step: int = -1
        for a in self.announcements:
            logger.info(f"Announcement scheduled for step {a['step']}: {a['message'][:80]}...")

        # Per-agent personas (optional, indexed by agent id)
        agent_config = self.config['agents']
        self.personas_config = agent_config.get('personas', None)
        if self.personas_config is not None:
            if len(self.personas_config) != self.num_agents:
                raise ValueError(
                    f"Number of personas ({len(self.personas_config)}) "
    ...
    def initialize_agents(self):
        ...
        # Determine genders + personas
        if self.personas_config is not None:
            genders = [p.get('gender', 'male') for p in self.personas_config]
            personas = [p.get('persona', '') for p in self.personas_config]
            logger.info(f"Using {self.num_agents} pre-defined personas from config.")
        else:
            half = self.num_agents // 2
            genders = ["male"] * half + ["female"] * (self.num_agents - half)
            random.shuffle(genders)
            personas = [""] * self.num_agents
    ...
    def step_simulation(self):
        ...
        # Announcement check: broadcast new announcement when its step arrives.
        # The current_announcement persists for AGENT_ANNOUNCEMENT_TTL steps,
        # then clears so it doesn't dominate forever.
        AGENT_ANNOUNCEMENT_TTL = 3
        for ann in self.announcements:
            if ann['step'] == self.step:
                self.current_announcement = ann['message']
                self.last_announcement_step = self.step
                logger.info(f"📢 ANNOUNCEMENT at step {self.step}: {ann['message']}")
                break
        # Auto-clear announcement after TTL
        if (self.current_announcement is not None and
                self.step - self.last_announcement_step > AGENT_ANNOUNCEMENT_TTL):
        ...
        for fc in self.fire_configs:
            if fc['name'] not in active_names and self.step >= fc['start_step']:
                ...
                logger.info(
                    f"PRESSURE DROP '{fc['name']}' started at position {fire_pos} with intensity "
        ...
        for agent in self.agents:
            ...
            message_decision = agent.decide_message(
                agent_place_status, nearby_agents, self.step,
                fire_info=fire_info,
        ...
        for agent, message_decision, nearby_agents in message_decisions:
            ...
            action_decision = agent.decide_action(
                agent_place_status, nearby_agents, self.step, message_content,
                fire_info=fire_info,
                announcement=self.current_announcement,
            )
        ...
        # Phase 4: Execute movement with capacity enforcement.
        # If the target cell is inside a room that is already at capacity and
        # the agent is *entering* (not already in that same room), the move
        # is blocked — the agent stays put this tick. Sequential processing
        # means earlier moves consume capacity before later ones are checked.
        for agent, action_decision in action_decisions:
            if action_decision['action'] != 'move' or not action_decision['direction']:
                continue

            direction = action_decision['direction']
            x, y = agent.position
            ...
            target_place = get_place_at_position((new_x, new_y), self.places)

            if target_place is not None:
                already_inside_same = (
                    agent.in_place and agent.current_place == target_place['name']
                )
                if not already_inside_same:
                    # Agent would be newly entering target_place — check capacity
                    current_count = sum(
                        1 for a in self.agents
                        if a.in_place and a.current_place == target_place['name']
                    )
                    if current_count >= target_place['capacity']:
                        logger.info(
                            f"Step {self.step}: Agent {agent.id} move to "
                            f"{target_place['name']} BLOCKED (at capacity "
                            f"{current_count}/{target_place['capacity']})"
                        )
                        continue
