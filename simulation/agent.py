...
class Agent:
    ...
    def __init__(
        ...
        self.world_description_override = world_description
    ...
    def _build_fire_section(self, fire_info: Optional[List[Dict]]) -> str:
        """Build hazard event section for prompt (pressure drop zones).

        Adds explicit hazard framing so agents interpret quantitative data
        as a real danger that warrants evacuation.
        """
        if not fire_info:
            return ""

        lines = [
            "\n=== ⚠️ HAZARD: PRESSURE DROP ZONE ===",
            "警告：圧力低下ゾーンの内部は人体に有害です。"
            "長時間の滞在は健康被害（意識朦朧、呼吸困難など）を引き起こす可能性があります。"
            "ゾーン内にいる場合は、速やかに外へ退避することを検討してください。",
        ]
        for fi in fire_info:
            lines.append(
                f"\nPressure drop zone \"{fi['name']}\":\n"
    ...
    def _build_persona_section(self) -> str:
        ...
        return (
            "\n=== YOUR PERSONA ===\n"
            f"{self.persona}\n"
            "Stay true to this persona in your messages, memory, and reasoning.\n"
        )

    def _build_announcement_section(self, announcement: Optional[str]) -> str:
        if not announcement:
            return ""
        return (
    ...
    def create_message_prompt(
        ...
        if self.in_place and place_status and current_place_info:
            ...
            place_description = current_place_info.get('description', '')
            ...
            place_section_text = (
                f"\nYou are currently in the {place_type} ({place_name}).{desc_line}"
        ...
        # Determine world description: config override takes priority
        if self.world_description_override:
            world_description = self.world_description_override
        else:
            place_types = [p['type'] for p in self.places]
            unique_types = list(set(place_types))
            if 'badminton_court' in unique_types and len(self.places) >= 2:
                world_description = (
                    "an indoor badminton facility (a single room covering the whole field). "
                    "Inside this room there are 2 badminton courts where actual play can happen. "
                    "The area outside the courts but inside the room is a shared observation, "
                    "waiting and chatting space. You cannot leave the room"
                )
            else:
        ...
        announcement_section = self._build_announcement_section(announcement)

        prompt = f"""You are Agent {self.id} ({self.gender}) in {world_description}.
{persona_section}{announcement_section}
=== YOUR CURRENT STATE ===
Gender: {self.gender}
...
=== ACTION IMPERATIVE ===
You are NOT a passive observer waiting for instructions.
You have full agency to act on your own initiative.

You can DO things at any time (describe these in your memory/reasoning):
- Walk into or out of the marked area in front of you
- Pick up, hold, or use any of the available equipment
- Try a physical movement / interact with the equipment
- Approach another agent and propose an activity together
- Observe what others are doing, comment, encourage, or coordinate
- Suggest formats, rules, configurations, or roles

If you find yourself "waiting for instructions" for several steps,
that's a sign to ACT INSTEAD. Make a concrete attempt.
**Inaction is boring. Choose to do something concrete.**
**Avoid abstract discussions about "the meaning of place" or "shared consciousness".**

=== YOUR TASK ===
Decide what message you want to send to nearby agents. You can share your
observations, experiences, intentions, or proposals about gameplay.

=== IMPORTANT: RESPONSE LANGUAGE ===
Write all text fields ("message" and "reasoning") in natural English.
Keep the JSON keys in English.

=== RESPOND IN JSON ===
{{
    "message": "message to nearby agents (in English, max 200 words, optional if you don't want to send a message)",
    "reasoning": "brief explanation of why you want to send this message (in English)"
}}
...
"""
    ...
    def create_decision_prompt(
        ...
        # Build place locations description (supports rectangular places)
        from utils import get_place_half_sizes
        place_locations = []
        for place in self.places:
            place_type = place['type']
            place_desc = place.get('description', '')
            desc_suffix = f"\n    Description: {place_desc}" if place_desc else ""
            hx, hy = get_place_half_sizes(place)
            place_locations.append(
                f"{place['name']} ({place_type}): center at ({place['center_x']}, {place['center_y']}), "
                f"covers X from {place['center_x'] - hx} to {place['center_x'] + hx}, "
                f"Y from {place['center_y'] - hy} to {place['center_y'] + hy}"
                f"{desc_suffix}"
            )
...
=== MESSAGES FROM OTHERS ===
{messages_text}
{message_section}=== ACTION IMPERATIVE ===
You are NOT a passive observer waiting for instructions.
You have full agency to act on your own initiative.
...
If you find yourself "waiting for instructions" for several steps,
that is a sign to ACT INSTEAD. Make a concrete attempt:
- Move toward something
- Try a physical action with the equipment
...
=== IMPORTANT: RESPONSE LANGUAGE ===
Write all text fields ("memory" and "reasoning") in natural English.
Keep JSON keys and action/direction values in English.

=== RESPOND IN JSON ===
{{
    "action": "move" or "stay",
    "direction": "up", "down", "left", or "right" (only if action is "move"),
    "memory": "what you want to remember for the next step (your thoughts, observations, intentions, in English)",
    "reasoning": "brief explanation of your decision (in English)"
}}
...
"""
    ...
    def decide_message(
        ...
        prompt = self.create_message_prompt(
            place_status, nearby_agents, step,
            fire_info=fire_info, announcement=announcement
        )
